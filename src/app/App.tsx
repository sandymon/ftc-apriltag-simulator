...
export default function App() {
  ...
  const visibleModules = courseFor(), lessons = visibleModules.flatMap(m => m.lessons);
  ...
  return <div className={"app " + progress.theme + (presenter ? " presenter" : "") + (progress.reducedMotion ? " reduce-motion" : "")}>
    ...
    <header className="app-header">
      ...
      <div className="header-center"><span className="header-divider"/><Icon name="book" size={16}/><span>30 minutes · camera to code</span></div>
    ...
    <div className={"app-layout " + (labOpen ? "with-lab" : "")}>
      <aside className={"course-sidebar " + (outline ? "open" : "")} aria-label="Course outline">
        <div className="sidebar-heading"><span className="eyebrow">THE 30-MINUTE WORKSHOP</span><span className="small-badge">{visibleModules.length} modules</span></div>
      ...
      <main className="course-main">
        ...
        <div className="lesson-layout"><article className={"lesson-stage kind-" + lesson.kind} id="lesson">
          <div className="slide-meta"><span className="eyebrow">{minuteStart + "–" + (minuteStart + module.minutes) + " MIN"} <i/> {lesson.kind === "code" ? "CODE WALKTHROUGH" : lesson.kind === "challenge" ? "HANDS-ON LAB" : "GUIDED LESSON"}</span><span>{String(module.lessons.indexOf(lesson) + 1).padStart(2, "0")} / {String(module.lessons.length).padStart(2, "0")}</span></div>
          ...
          {lesson.kind === "code" && <CodeWalkthrough key={lesson.id} essentials focus={lesson.focus} onDemonstrate={demonstrate}/>}
          {lesson.kind === "measurements" && <TargetDataGuide camera={lesson.focus === "webcam" ? "webcam" : "limelight"}/>}
          {lesson.kind === "ui" && <UIExplainer key={lesson.id} focus={lesson.focus} essentials/>}
          ...
          {lesson.kind === "resources" && <Resources/>}
          {lesson.id === "explore-lab" && <button className="button primary" onClick={openLab}><Icon name="lab" size={16}/> Explore lab</button>}
